...
struct SelectionInfoForXref
{
	...
	bool operator==(const SelectionInfoForXref& other) const
	{
		if (addrValid && other.addrValid)
			return (start == other.start) && (end == other.end) && (func == other.func) &&
			       (arch == other.arch);
...
class BINARYNINJAUIAPI View
{
 protected:
	...
	// FIXME: Support for typeview, where the default navigation mode is not compatible with the
	// navigation interface The view concept and navigation interface needs to be revisited at some
	// point New interface/design should be pushed to NavigationHandler and through API The empty
	// string is global navigation (inside view) by default, allows offset to be interpreted by mode
 ...
 public:
	...
	virtual bool findAllData(uint64_t start, uint64_t end, const BinaryNinja::DataBuffer& data,
	    BNFindFlag flags, const std::function<bool(size_t current, size_t total)>& cb,
	    const std::function<bool(uint64_t addr, const BinaryNinja::DataBuffer& match)>&
	        matchCallback);
	...
	virtual bool findAllConstant(uint64_t start, uint64_t end, uint64_t constant,
	    DisassemblySettingsRef settings, BNFunctionGraphType graph,
	    const std::function<bool(size_t current, size_t total)>& cb,
	    const std::function<bool(uint64_t addr, const BinaryNinja::LinearDisassemblyLine& line)>&
	        matchCallback);
...
class BINARYNINJAUIAPI ViewLocation
{
 ...
 public:
	...
	ViewLocation(const QString& viewType, uint64_t offset) :
	    m_valid(true), m_viewType(viewType), m_offset(offset)
	{}
	...
	{}
	ViewLocation(
	    const QString& viewType, uint64_t offset, BNFunctionGraphType ilViewType, size_t instrIndex) :
	    m_valid(true),
	    m_viewType(viewType), m_offset(offset), m_ilViewType(ilViewType), m_instrIndex(instrIndex)
	{}
	ViewLocation(const QString& viewType, FunctionRef function, uint64_t offset,
	    BNFunctionGraphType ilViewType, size_t instrIndex) :
	    m_valid(true),
	    m_viewType(viewType), m_function(function), m_offset(offset), m_ilViewType(ilViewType),
	    m_instrIndex(instrIndex)
	{}
	ViewLocation(
	    FunctionRef function, uint64_t offset, BNFunctionGraphType ilViewType, size_t instrIndex) :
	    m_valid(true),
	...
	{
		return (m_valid == other.m_valid) && (m_viewType == other.m_viewType) &&
		       (m_offset == other.m_offset) && (m_ilViewType == other.m_ilViewType) &&
		       (m_instrIndex == other.m_instrIndex) && (m_function == other.m_function);
	}
...
class BINARYNINJAUIAPI ViewFrame : public QWidget
{
 ...
 private:
	QWidget* createView(const QString& typeName, ViewType* type, BinaryViewRef data,
	    bool createDynamicWidgets = true);
	BinaryNinja::Ref<HistoryEntry> getHistoryEntry();

	FileContext* m_context;
	bool m_fileContentsLock =
	    true;  // file contents protection from accidental modification in the UI
 ...
 public:
	explicit ViewFrame(
	    QWidget* parent, FileContext* file, const QString& type, bool createDynamicWidgets = false);
	virtual ~ViewFrame();
	...
	bool navigate(
	    const QString& type, uint64_t offset, bool updateInfo = true, bool addHistoryEntry = true);
	bool navigate(const QString& type, const std::function<bool(View*)>& handler,
	    bool updateInfo = true, bool addHistoryEntry = true);
	bool navigate(
	    BinaryViewRef data, uint64_t offset, bool updateInfo = true, bool addHistoryEntry = true);
	bool navigateToFunction(
	    FunctionRef func, uint64_t offset, bool updateInfo = true, bool addHistoryEntry = true);
	...
	bool navigateToViewLocation(
	    BinaryViewRef data, const ViewLocation& viewLocation, bool addHistoryEntry = true);
	...
	/*!
	    Parse history entries from the raw data associated with a BinaryView, loading them into the
	   back/forward navigation stacks, and navigating to the saved position. \param data View
	   containing history entries
	 */
	void readHistoryEntries(BinaryViewRef data);
	/*!
	    Serialize history entries and current position, storing them in the raw data associated with a
	   BinaryView. \param data View for saving history entries
	 */
	...
	static bool getAddressFromInput(QWidget* parent, BinaryViewRef data, uint64_t& offset,
	    uint64_t currentAddress, const QString& title = "Go to Address",
	    const QString& msg = "Address:", bool defaultToCurrent = false);
	static bool getFileOffsetFromInput(QWidget* parent, BinaryViewRef data, uint64_t& offset,
	    uint64_t currentAddress, const QString& title = "Go to File Offset",
	    const QString& msg = "File Offset:", bool defaultToCurrent = false);
